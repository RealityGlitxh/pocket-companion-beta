/* PocketNexus V8.64.2 — closed beta validation service worker */
const VERSION='v8.64.2-card-detail-enrichment1';
...
async function networkFirst(req,cacheName){const c=await caches.open(cacheName);try{const res=await fetch(req);if(res&&res.ok){c.put(req,res.clone());trim(cacheName,80);}return res;}catch{const cached=await c.match(req);return cached||await caches.match(req)||Response.error();}}
...
self.addEventListener('fetch',event=>{
 ...
 if(req.mode==='navigate'){event.respondWith(networkFirst(req,RUNTIME_CACHE));return;}
 if(req.destination==='script'||req.destination==='style'){event.respondWith(networkFirst(req,RUNTIME_CACHE));return;}
 event.respondWith(caches.match(req).then(cached=>cached||fetch(req).then(res=>{if(res.ok)caches.open(RUNTIME_CACHE).then(c=>c.put(req,res.clone()));return res;}).catch(()=>Response.error())));
});
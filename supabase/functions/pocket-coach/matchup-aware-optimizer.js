export function buildMatchupAwareOptimizations({decks=[],metaAware=[],snapshot=null,archetypes=[],snapshotMatchups=[],cardEvidence=[]}={}){
  ...
  return arr(decks).map(deck=>{
    ...
    const enriched=priorities.map(p=>{
      ...
      for(const x of usefulAdds.slice(0,2))recommendations.push({action:'consider-matchup-tech',cardName:x.cardName,set:x.set,number:x.number,opponentArchetype:p.opponentName,reason:'positive-result-association-in-this-matchup',confidence:x.evidence==='high'&&p.evidence==='high'?'high-directional':'medium-directional',evidence:{matchupGames:x.matchupGames,cardGames:x.cardGames,inclusionRatePct:x.inclusionRate,cardWinRate:x.cardWinRate,baselineWinRate:x.baselineWinRate,deltaWinRate:x.deltaWinRate},guardrail:'This is an association in observed lists, not proof the card caused the better result.'});
      for(const x of reviewPresent.slice(0,2))recommendations.push({action:'review-for-matchup',cardName:x.cardName,set:x.set,number:x.number,opponentArchetype:p.opponentName,reason:'negative-result-association-in-this-matchup',confidence:'review-only',evidence:{matchupGames:x.matchupGames,cardGames:x.cardGames,inclusionRatePct:x.inclusionRate,cardWinRate:x.cardWinRate,baselineWinRate:x.baselineWinRate,deltaWinRate:x.deltaWinRate},guardrail:'Do not cut this card solely from correlation; role, matchup plan, and structural needs still matter.'});

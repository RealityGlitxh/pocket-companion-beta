...
function compactDeckPayload(d:any){
  ...
  return {
    id:d?.id||d?.localId||'',name:d?.name||'Untitled deck',archetype:d?.archetype||d?.archetypeName||'',energy:d?.energy||d?.energyType||'',
...
function aggregateMatches(matches:any[]){
  ...
  for(const m of matches){
    ...
    for(const [map,key] of [[byDeck,d],[byOpponent,o]] as any){
      ...
      const row=map.get(key);row.matches++;if(result==='win')row.wins++;else if(result==='loss')row.losses++;else row.ties++;
...
function fallbackAnswer(message:string,ctx:any){
  ...
  if(lower.includes('meta')){
    const top=ctx?.meta?.top||[];return top.length?`The latest stored Meta snapshot is led by ${top.slice(0,3).map((x:any)=>`${x.name} (${Number(x.usage_pct||0).toFixed(1)}%)`).join(', ')}. Use these as preparation priorities, not guarantees.`:'There is no ready Meta snapshot available to ground that answer right now.';
  }
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const {data:{user},error:userErr}=await authClient.auth.getUser();if(userErr||!user)return json({error:'Unauthorized'},401);
    ...
    const message=String(body?.message||'').trim().slice(0,4000);conversationId=body?.conversationId||null;if(!message)return json({error:'Message is required'},400);

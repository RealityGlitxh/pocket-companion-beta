...
assert(/body\.scope\s*===?\s*['"]lookup['"]/.test(proxy),'Limitless refresh Edge Function does not support live lookup mode');
assert(proxy.includes('lookupFromPages'),'Limitless refresh Edge Function does not include the live-page fallback');
assert(proxy.includes('/details`')&&proxy.includes('/standings`')&&proxy.includes('/pairings`'),'Limitless lookup paths are incomplete');
assert(proxy.includes('parseStandings')&&proxy.includes('parseBracketPairings'),'Live public page parsers are missing');
...
const limitlessStub={tournamentId(input){const raw=String(input??'').trim();const m=raw.match(/(?:tournament\/|tournaments\/)([a-zA-Z0-9_-]+)/i);return m?m[1]:raw.replace(/^\/+|\/+$/g,'').split('/')[0]},rounds(){return []},async fetchTournament(){return {id:'stub',details:{name:'Live Tournament',status:'ongoing'},players:[],pairings:[]}}};
const context={console,URL,window:{},document,state:{page:'dashboard',streamer:{}},MutationObserver,requestAnimationFrame(fn){fn()},setInterval(){return 1},clearInterval(){},setTimeout,clearTimeout,PPCLimitlessLiveTable:limitlessStub};
context.window=context;vm.createContext(context);vm.runInContext(src,context,{filename:featurePath});
const test=context.PPCStreamerTournamentAutoLink?._test;assert(test,'Feature did not expose test helpers');

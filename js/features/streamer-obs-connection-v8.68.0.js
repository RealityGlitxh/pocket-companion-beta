/* PocketNexus V8.68.1 — external OBS heartbeat tracking.
   Embedded PocketNexus previews no longer impersonate a real OBS browser source. */
(function(){
 ...
 for(const m of modes)try{const ch=new BroadcastChannel('ppc-stream-v868-'+m);ch.onmessage=e=>{const t=e.data?.type;if((t==='overlay-ready'||t==='pong'||t==='overlay-heartbeat')&&e.data?.embedded!==true){seen[m]=Date.now();paint()}};setInterval(()=>{try{ch.postMessage({type:'ping',from:'app',at:Date.now()})}catch{}},3000)}catch{}
 setInterval(paint,1000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)paint()});
 window.PPCOBS2Connection={version:'8.68.1',seen,paint};
})();
...
(function(){
 ...
 async function sharedDeckLanding(){
  ...
  try{const {data,error}=await client.rpc('get_shared_deck',{p_share_token:token});if(error)throw error;const row=Array.isArray(data)?data[0]:data;if(!row)throw new Error('Shared deck not found or revoked.');
   const p=row.deck_payload||{},app=document.getElementById('app');if(app){app.innerHTML=`<div class="v849ShareLanding"><section class="panel v849Shared"><span class="eyebrow">SHARED DECK</span><h1>${esc(row.title||p.name||'Shared Deck')}</h1><p class="muted">${esc(p.archetype||p.energy||'Pokémon TCG Pocket')}</p><div>${(p.cards||[]).map(c=>`<span><b>${Number(c.qty||1)}×</b> ${esc(c.name||c.id||'Card')}</span>`).join('')||'<p>No cards available.</p>'}</div><button class="secondary" onclick="location.href=location.pathname">Open PocketNexus</button></section></div>`;document.body.classList.add('sharedDeckMode');}
  }catch(e){console.warn('V8.50 shared deck',e);const app=document.getElementById('app');if(app)app.innerHTML=`<section class="panel"><h2>Shared deck unavailable</h2><p class="muted">This link may have been revoked or expired.</p><button onclick="location.href=location.pathname">Open PocketNexus</button></section>`;}
 }

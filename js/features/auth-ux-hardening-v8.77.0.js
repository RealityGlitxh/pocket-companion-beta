/* PocketNexus v8.77.0 — Account authentication UX hardening.
...
(function(){
  ...
  function toggleAuthPassword(inputId='authPassword',button){
    ...
    const btn=button||document.getElementById('authPasswordToggle');
    if(!input||!btn)return;
  ...
  function enhanceAccountAuthUI(){
    ...
    if(!document.getElementById('authPasswordToggle')){
      ...
      toggle.type='button';
      toggle.className='secondary authPasswordToggle';
      toggle.setAttribute('aria-label','Show password');
      toggle.setAttribute('aria-pressed','false');
      toggle.textContent='Show';
      toggle.addEventListener('click',()=>toggleAuthPassword('authPassword',toggle));
  ...
  async function hardenedEmailSignIn(){
    ...
    if(typeof originalEmailSignIn!=='function')return;
    signInPending=true;
    setButtonBusy('accountEmailSignInBtn',true,'Signing in…','Sign In');
    try{
      await originalEmailSignIn();
      translateMessage('sign-in');
    }catch(e){
      const el=document.getElementById('authMessage');
      if(el){el.className='dangerBox';el.textContent=friendlyAuthMessage(e?.message,'sign-in')}
    }finally{
      signInPending=false;
      setButtonBusy('accountEmailSignInBtn',false,'Signing in…','Sign In');
    }
  }

  async function hardenedEmailSignUp(){
    if(signUpPending)return;
    if(typeof originalEmailSignUp!=='function')return;
    signUpPending=true;
    try{
      await originalEmailSignUp();
      translateMessage('sign-up');
    }catch(e){
      const el=document.getElementById('authMessage');
      if(el){el.className='dangerBox';el.textContent=friendlyAuthMessage(e?.message,'sign-up')}
  ...
  window.accountPage=hardenedAccountPage;
  window.emailSignIn=hardenedEmailSignIn;
  window.emailSignUp=hardenedEmailSignUp;
  window.toggleAuthPassword=toggleAuthPassword;
  window.PPCAuthUXHardening={version:'8.77.0',enhance:enhanceAccountAuthUI,friendlyAuthMessage};
})();

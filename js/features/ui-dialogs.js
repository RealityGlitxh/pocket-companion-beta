(function(){
  ...
  function open({eyebrow='',title='',message='',html='',actions=[]}={}){
    ...
    b.innerHTML=`<div class="ppcDialog"><div class="between"><div>${eyebrow?`<span class="eyebrow">${eyebrow}</span>`:''}<h2>${title||'Pocket Companion'}</h2></div><button class="secondary" type="button" onclick="PPCUI.close()">Close</button></div>${message?`<p class="muted">${message}</p>`:''}${html||''}<div class="row ppcDialogActions">${actions.map(a=>`<button type="button" class="${a.className||''}" onclick="${a.onclick||'PPCUI.close()'}">${a.label||'OK'}</button>`).join('')}</div></div>`;
  ...
  window.alert=function(message){notice(message,{title:'Pocket Companion'});};
})();

...
function streamerPage(){
 ...
 document.getElementById("app").innerHTML=`<div class="between"><div><h1>Stream Control Center</h1><p class="muted">Ranked • Tournament • Caster scenes + OBS tools</p></div><span class="badge">STREAM READY</span></div>
...
function fullDiagnosticResults(){
 ...
 add("Collection → Deck Lab integration",typeof deckCollectionStatus==="function"&&typeof openMissingCards==="function"&&typeof addAllMissingToWishlist==="function","Live calculations available");
 add("Better Battle Tracker",typeof quickRecordResult==="function"&&typeof quickRematch==="function"&&typeof undoLastBattleMatch==="function","Quick record, rematch, and undo available");
 add("First-time onboarding",!!state.onboarding&&typeof onboardingRender==="function"&&typeof onboardingRestart==="function","First-time setup available");
 add("Personal Meta",typeof personalMetaMatrixHtml==="function","Feature available");
 add("Tournament Prep",typeof tournamentPrepHtml==="function","Feature available");
 add("Rank season lifecycle safeguard",typeof rankSeasonLifecycle==="function","Season expiry safeguard available");
...
function rankBorderQuickSummary(border,seasonLife){
 ...
 if(!seasonLife.live)return `<section class="panel rankQuickSummary"><div class="between"><div><span class="eyebrow">COMPLETED SEASON</span><h2>${esc(rankBorderLabel(border.targetRank))}</h2><p class="muted">These values are preserved as historical results from the completed season. They are not an active RP goal.</p></div><span class="confidence ${esc(String(border.confidence||"low").toLowerCase())}">${esc(rankBorderConfidence(border.confidence))} confidence</span></div><div class="rankQuickGrid"><div><span>Your tracked RP</span><strong>${rankBorderFmt(rp)}</strong><small>Current PocketNexus value</small></div><div><span>Final estimate</span><strong>${rankBorderFmt(target)}</strong><small>${esc(rankBorderLabel(border.targetRank))}</small></div><div><span>Last estimated target</span><strong>${rankBorderFmt(safe)}</strong><small>Historical — season ended</small></div></div><div class="notice"><strong>Season ended.</strong> No RP-to-go target is active until the next ranked season begins.</div></section>`;
 return `<section class="panel rankQuickSummary"><div class="between"><div><span class="eyebrow">YOUR CLIMB</span><h2>${esc(rankBorderLabel(border.targetRank))}</h2><p class="muted">The shortest answer first. Forecasts below are independent estimates, not official Pokémon leaderboard values.</p></div><span class="confidence ${esc(String(border.confidence||"low").toLowerCase())}">${esc(rankBorderConfidence(border.confidence))} confidence</span></div><div class="rankQuickGrid"><div><span>Confirmed RP</span><strong>${rankBorderFmt(rp)}</strong><small>Updates from ranked Battle Log results</small></div><div><span>Estimated finish</span><strong>${rankBorderFmt(target)}</strong><small>${esc(rankBorderLabel(border.targetRank))}</small></div><div><span>Estimated safe target</span><strong>${rankBorderFmt(safe)}</strong><small>${above?"You are above it":`${rankBorderFmt(delta)} RP to go`}</small></div></div><div class="${above?'successBox':'notice'}"><strong>${above?'✓ You are above the estimated safety target.':`About ${rankBorderFmt(delta)} RP to the estimated safety target.`}</strong> Check Pokémon TCG Pocket for your official rank and RP.</div></section>`;
}
...
function aboutPage(){
 ...
 document.getElementById("app").innerHTML=`<div class="between"><div><span class="eyebrow">CLOSED BETA VALIDATION</span><h1>About, Privacy & Beta Status</h1><p class="muted">Validate the release candidate on real hosted devices before public launch.</p></div><button class="secondary" onclick="goPage('account')">Account & Data</button></div>
 <section class="panel"><div class="between"><div><span class="eyebrow">RELEASE READINESS</span><h2>Closed beta status</h2></div><span class="badge">CLOSED BETA</span></div><div class="launchStatusGrid"><article><span>Connection</span><strong>${online?'Online':'Offline'}</strong><small>Current browser state</small></article><article><span>Secure context</span><strong>${secure?'Ready':'Needs HTTPS'}</strong><small>PWA/auth production requirement</small></article><article><span>Session</span><strong>Closed Beta</strong><small>${esc(session?.id||'Beta session')}</small></article></div><div class="launchLinks"><a class="buttonLink secondary" href="privacy.html">Privacy</a><a class="buttonLink secondary" href="terms.html">Terms</a><a class="buttonLink secondary" href="support.html">Support</a></div></section>
 ${betaQaHtml()}
...
function cloudDomainCenter848(){
 ...
 if(!rows.length){setTimeout(async()=>{await loadCloudDomainSummary848();if(state.page==='account')accountPage()},0);return `<section class="panel v848SyncCenter"><span class="eyebrow">ACCOUNT & CLOUD</span><h2>Sync Center</h2><p class="muted">Loading your cloud data…</p></section>`}
 return `<section class="panel v848SyncCenter"><div class="between"><div><span class="eyebrow">ACCOUNT & CLOUD</span><h2>Sync Center</h2><p class="muted">See which parts of PocketNexus are stored in your account.</p></div><button class="secondary" onclick="refreshCloudDomainSummary848()">Refresh</button></div><div class="v848DomainGrid">${rows.map(r=>`<div class="v848Domain"><div class="between"><strong>${esc(syncDomainLabel848(r.domain))}</strong><span class="badge">${esc(r.sync_status||'idle')}</span></div><div class="v848DomainCount">${Number(r.item_count||0)}</div><small class="muted">cloud items${Number(r.deleted_count||0)?` • ${Number(r.deleted_count)} archived`:''}</small>${r.last_cloud_change_at?`<small>Updated ${new Date(r.last_cloud_change_at).toLocaleString()}</small>`:''}${r.last_error?`<small class="dangerText">${esc(r.last_error)}</small>`:''}</div>`).join('')}</div><p class="muted tiny">Decks now support cloud tombstones too, matching Battle, Rank, and Collection recovery behavior.</p></section>`;
}

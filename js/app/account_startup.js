...
function onboardingRender(){
 ...
 if(step===0){
  ...
  html=onboardingShell(`<div class="onboardingEmpty"><h2>Welcome to Pokémon Pocket Companion</h2><p>Track your matches. Understand your decks. Study the Meta. Improve your performance.</p></div><h3>What do you want to focus on?</h3><div class="onboardingChoices">${goals.map(g=>`<button class="onboardingChoice ${state.onboarding.goal===g?'selected':''}" onclick="onboardingSetGoal('${g}')">${g}</button>`).join('')}</div>`,`Welcome, ${state.user||'Trainer'}`,"A quick setup will connect the app around the way you play.");
 }else if(step===1){
...
function startupFailureScreen(error){
 console.error("Pocket Companion startup failed",error);
 const root=document.getElementById("app");
 const navRoot=document.getElementById("nav");
 if(navRoot)navRoot.innerHTML="";
 if(root)root.innerHTML=`<div class="panel startupError" style="max-width:680px;margin:50px auto"><span class="badge">Startup Recovery</span><h1>Pocket Companion could not finish loading</h1><p class="muted">Your data has not been deleted. A startup error was caught instead of leaving a blank page.</p><div class="dangerBox"><strong>Error</strong><br>${esc(error?.message||String(error||"Unknown startup error"))}</div><div class="row" style="margin-top:12px"><button onclick="location.reload()">Reload App</button><button class="secondary" onclick="resetStartupPageOnly()">Open Local Welcome Screen</button></div></div>`;
}

...
function rankSessionPanel(){
 ...
 const svc=rankSessionService(),liveSession=svc?.getData?.()?.session,activeSession=!!cloudSession?.user&&!!liveSession&&!rankSessionLoading;
 const quickWL=activeSession?`<div class="rankSessionQuickWL"><span class="muted tiny">Quick result for this session</span><div class="row"><button class="rankQuickWinBtn" onclick="rankSessionRecord('win')">+ Win</button><button class="rankQuickLossBtn secondary" onclick="rankSessionRecord('loss')">+ Loss</button></div></div>`:"";
 return `<div class="panel rankSessionPanel"><div class="between"><div><span class="eyebrow">RANK TRACKING</span><h2>${rankBorderFmt(rp)} RP</h2><p class="muted">Confirmed RP + your recorded ranked results.</p></div><span class="pill">AUTO RP</span></div><div class="metricgrid"><div class="metric"><div class="l">Current RP</div><div class="n">${rankBorderFmt(rp)}</div><small>Updates from ranked Battle Log results</small></div><div class="metric"><div class="l">Current Streak</div><div class="n">${st.type==="win"?'W'+rankBorderFmt(st.count):'W0'}</div><small>Result streak only</small></div><div class="metric"><div class="l">Best Win Streak</div><div class="n">W${rankBorderFmt(st.bestWin||0)}</div></div><div class="metric"><div class="l">Ranked Record</div><div class="n">${rec.w}-${rec.l}</div><small>${rec.n?rec.wr.toFixed(1)+'% WR':'No ranked games yet'}</small></div></div>${quickWL}<div class="rankSessionActions"><button onclick="goPage('matches')">Record Ranked Match</button><button class="secondary" onclick="state.page='more';render()">Edit Current RP</button></div>${rankSessionNotice?`<div class="notice" style="margin-top:12px">${esc(rankSessionNotice)}</div>`:""}<div class="notice" style="margin-top:12px"><strong>Ranked Battle Log results update RP automatically.</strong> Wins add 10 RP plus streak bonus; losses use the current rank deduction. You can still enter the official post-match RP in Detailed mode to correct any difference.</div></div>`;
}
...
function rankBorderPage(){
 ...
 if(!data){const emblem=typeof window.rankEmblemHtml==="function"?window.rankEmblemHtml(state.rank?.tier,56):"";root.innerHTML=`<div class="between"><div class="rankBorderHeaderIdentity">${emblem?`<div class="rankBorderEmblemWrap">${emblem}</div>`:""}<div><span class="badge">RANK ESTIMATES</span><h1>${esc(state.rank?.tier||"Rank Intelligence")}</h1><p class="muted">Independent ranked-border estimates and finish projections. Not official Pokémon rankings.</p></div></div>${rankBorderSourceBadge()}</div><div class="rankBorderSkeleton"><div class="panel skeletonBlock"></div><div class="panel skeletonBlock"></div></div>${st.error?`<div class="dangerBox">${esc(st.error)}</div>`:""}`;return}
 ...
 const rankEmblem=typeof window.rankEmblemHtml==="function"?window.rankEmblemHtml(state.rank?.tier,64):"";
 root.innerHTML=`<div class="between rankBorderHeader"><div class="rankBorderHeaderIdentity">${rankEmblem?`<div class="rankBorderEmblemWrap">${rankEmblem}</div>`:""}<div><span class="eyebrow">RANKED PLAY</span><h1>${esc(state.rank?.tier||"Rank Details")}</h1><p class="muted">${esc(seasonInfo.name||season)} • ${esc(countdown)}</p></div></div><div class="row">${rankBorderSourceBadge()}<button class="secondary" onclick="rankBorderRefresh()">Refresh Borders</button></div></div>${st.error?`<div class="warningBox">Live refresh failed. Showing cached border data. ${esc(st.error)}</div>`:""}${seasonNotice}${rankBorderQuickSummary(selected,seasonLife)}${rankSessionPanel()}<details class="rankAdvancedDetails"><summary class="panel rankAdvancedSummary"><div><span class="eyebrow">ADVANCED RANK INTELLIGENCE</span><strong>Forecasts, chart, methodology & observations</strong><small>Open only when you want the full model details.</small></div><span>View details ↓</span></summary><div class="rankAdvancedBody">${cards}${historicalPanel}${rankBorderPersonalCard(selected)}${rankBorderChart(data)}${method}${observations}</div></details>`;
}

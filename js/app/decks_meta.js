...
${metaMatchupPanel()}${metaComparePanel()}<div class="bottomnote">Competitive data compiled from publicly available tournament information. PocketNexus is an independent third-party companion and is not affiliated with The Pokémon Company.</div>`;
}
...
function legacyMetaPage(){
 ...
 document.getElementById("app").innerHTML=`<div class="between"><div><h1>Meta Center</h1><p class="muted">Competitive snapshot analytics, verified sample decks, matchups, card usage, tournament finishes, and your personal results.</p></div><div><span class="badge">Competitive Snapshot</span><div class="muted tiny" style="margin-top:6px">Generated ${new Date(BTM_META_SNAPSHOT.generatedAt).toLocaleString()}</div></div></div><div class="metaTabsV73">${tabs.map(([k,n])=>`<button class="secondary ${tab===k?"active":""}" onclick="setMetaV73Tab('${k}')">${n}</button>`).join("")}</div>${body}<div class="panel sourcePanel"><strong>Snapshot status</strong><p class="muted">This section uses a frozen curated competitive snapshot for stable reference data. It is not a live feed; use Combined Competitive Meta above for the broader current field read.</p></div>`;
...
function openRenameDeckModal(id){
 ...
 const energies=["","Grass","Fire","Water","Lightning","Psychic","Fighting","Darkness","Metal","Dragon","Colorless"];
 body.innerHTML=`<div class="between"><div><h2>Quick Edit</h2><p class="muted">The deck keeps the same ID, so cloud sync updates the existing deck.</p></div><button class="secondary" onclick="closeCardModal()">Close</button></div><label for="renameDeckInput">Deck name</label><input id="renameDeckInput" maxlength="80" value="${esc(d.name||'')}" autocomplete="off"><label for="quickEditEnergy" style="margin-top:10px">Energy type</label><select id="quickEditEnergy">${energies.map(e=>`<option value="${esc(e)}" ${d.energy===e?"selected":""}>${e||"Not set"}</option>`).join("")}</select><div class="row" style="margin-top:14px"><button onclick="commitDeckRename('${esc(id)}')">Save</button><button class="secondary" onclick="closeCardModal()">Cancel</button></div>`;modal.style.display="flex";setTimeout(()=>{const el=document.getElementById('renameDeckInput');el?.focus();el?.select()},0);
}
function commitDeckRename(id){const d=state.decks.find(x=>x.id===id);const el=document.getElementById("renameDeckInput");const name=(el?.value||"").trim();if(!d||!name)return;if(state.decks.some(x=>x.id!==id&&String(x.name||"").toLowerCase()===name.toLowerCase())){el.insertAdjacentHTML("afterend",`<div class="dangerBox" style="margin-top:8px">A deck with that name already exists.</div>`);return}d.name=name;const energyEl=document.getElementById("quickEditEnergy");if(energyEl)d.energy=energyEl.value;save();closeCardModal();if(state.selected===id&&state.page==="decks")renderEditorShell(d);else render()}
...
function quickEditDeck(id){openRenameDeckModal(id)}

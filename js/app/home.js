...
function dashboardRankWidget(){
 dashboardEnsureRank();
 const svc=rankBorderService?.(),data=svc?.getData?.(),borders=Array.isArray(data?.borders)?data.borders.filter(b=>b.available):[];
 const userRP=Number(state.rank?.points||0),target=borders.find(b=>b.targetRank===1000)||borders[0];
 if(!data||!target)return `<section class="panel commandCard rankCommand"><div class="commandCardHead"><div><span class="eyebrow">RANK INTELLIGENCE</span><h2>Your climb</h2></div><span class="statusDot">SYNCING</span></div><div class="commandEmpty">Rank border data is loading. Your tracked RP is <strong>${rankBorderFmt(userRP)} RP</strong>.</div><button class="secondary commandLink" onclick="goPage('rank')">Open Rank Intelligence →</button></section>`;
 const safe=Number(target.recommendedSafeRP),pred=Number(target.predictedFinalRP),gap=Number.isFinite(safe)?userRP-safe:null;
 const life=rankSeasonLifecycle(data.season||{}),forecastLabel=life.live?"Forecast":"Final forecast",safeLabel=life.live?"Safe target":"Last safe target";
 return `<section class="panel commandCard rankCommand"><div class="commandCardHead"><div><span class="eyebrow">RANK INTELLIGENCE</span><h2>${esc(rankBorderLabel(target.targetRank))} outlook</h2></div><span class="statusDot ${life.live?'live':''}">${life.live?'LIVE':'ENDED'}</span></div><div class="commandMetric"><strong>${rankBorderFmt(userRP)}</strong><span>YOUR RP</span></div><div class="commandStatRow"><div><span>${forecastLabel}</span><b>${rankBorderFmt(pred)}</b></div><div><span>${safeLabel}</span><b>${rankBorderFmt(safe)}</b></div><div><span>Gap</span><b class="${gap>=0?'good':'bad'}">${gap>=0?'+':''}${rankBorderFmt(gap)}</b></div></div><div class="progressTrack"><i style="width:${Math.max(4,Math.min(100,safe?userRP/safe*100:0))}%"></i></div><p class="muted tiny">${esc(data.season?.name||'Ranked season')} • ${esc(life.label)}</p><button class="secondary commandLink" onclick="goPage('rank')">Open Rank Intelligence →</button></section>`;
}
...
function dashboard(){
 ...
 <section class="foundationHubGrid" aria-label="Main destinations">
  <article class="foundationHubCard playHub"><div class="hubCardTop"><span class="hubGlyph">◉</span><span class="eyebrow">PLAY</span></div><h2>Battle & Rank</h2><p>Record a match, continue your ranked climb, or run a Gym Battle.</p><div class="hubActions"><button onclick="goPage('matches')">Record Battle</button><button class="secondary" onclick="goPage('rank')">Open Rank</button></div><div class="hubStat"><span>Current RP</span><strong>${rankBorderFmt(rank.points)}</strong></div></article>

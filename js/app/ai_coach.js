...
async function coachSend(messageOverride=''){
 ...
 const c=coachClient(),s=coachSession();if(!c||!s?.user){pocketCoachState.error='Sign in to use Pocket Coach so it can securely read your Pocket Companion data.';return pocketCoachPage(true)}
 ...
 root.innerHTML=`<div class="coachPage">
  ...
  <div class="coachLayout">
   ...
   <section class="panel coachMain"><div class="coachTopBar"><div><span class="eyebrow">GROUNDED AI COACHING</span><h2>${pocketCoachState.conversationId?'Conversation':'Start a new conversation'}</h2></div><span class="pill">No invented game data</span></div>
    ...
    <div class="coachThread" id="coachThread">${pocketCoachState.messages.length?pocketCoachState.messages.map(coachMessageHtml).join(''):`<div class="coachWelcome"><div class="coachOrb">✦</div><h2>What do you want to improve?</h2><p>${providerGood?`Pocket Coach is connected to ${esc(pocketCoachState.model)} through the secure Supabase backend. Ask a competitive question and I’ll combine the model with your synced Pocket Companion context.`:`The real AI backend is installed and ready. Until the server-side OpenAI key is added, Pocket Coach automatically falls back to grounded rule-based answers instead of exposing a key in the browser.`}</p></div>`}${pocketCoachState.loading?`<article class="coachMessage assistant"><div class="coachAvatar">✦</div><div class="coachBubble coachThinking"><strong>${providerGood?'Thinking with your Pocket Companion context…':'Reading your Pocket Companion data…'}</strong><span></span><span></span><span></span></div></article>`:''}</div>

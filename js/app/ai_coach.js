/* V8.68.6 — Pocket Coach persistent history + training weakness integration */
 ...
 root.innerHTML=`<div class="coachPage">
  ...
  <div class="coachLayout">
   ...
   <section class="panel coachMain"><div class="coachTopBar"><div><span class="eyebrow">GROUNDED AI COACHING</span><h2>${pocketCoachState.conversationId?'Conversation':'Start a new conversation'}</h2></div><span class="pill">No invented game data</span></div>
    ...
    <div class="coachThread" id="coachThread">${pocketCoachState.messages.length?pocketCoachState.messages.map(coachMessageHtml).join(''):`<div class="coachWelcome"><div class="coachOrb">✦</div><h2>What do you want to improve?</h2><p>${providerGood?`Pocket Coach is connected to ${esc(pocketCoachState.model)} through the secure Supabase backend. Ask a competitive question and I’ll combine the model with your synced PocketNexus context, including Brain Teaser performance.`:`The real AI backend is installed and ready. Until the server-side OpenAI key is added, Pocket Coach automatically falls back to grounded rule-based answers instead of exposing a key in the browser.`}</p></div>`}${pocketCoachState.loading?`<article class="coachMessage assistant"><div class="coachAvatar">✦</div><div class="coachBubble coachThinking"><strong>${providerGood?'Comparing your training and match data…':'Reading your PocketNexus data…'}</strong><span></span><span></span><span></span></div></article>`:''}</div>
    ...
    <div class="coachGuardrail"><strong>Grounding rule:</strong> Brain Teaser mistakes are practice signals, not proof that the same mistake happened in a real match. Pocket Coach should distinguish training evidence from Battle Tracker evidence.</div>
   </section>
  </div>
 </div>`;
